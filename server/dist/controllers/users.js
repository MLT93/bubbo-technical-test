...
const signUp = async (req, res) => {
    const { username, password, email } = req.body;
    const user = await firebase.oneOrNone(`SELECT * FROM users WHERE username=$1`, username);
    if (user) {
        res.status(409).json({ msg: `Username already exist` });
    }
    else {
        const { id } = await firebase.one(`INSERT INTO users (username, password, email) VALUES ($1, $2, $3) RETURNING user_id`, [username, password, email]);
